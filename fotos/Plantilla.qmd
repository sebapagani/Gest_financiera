...
El siguiente diagrama explica el modelo
...
Gastón Silverio, en su paper "Modelo binomial para la valoración de empresas y los efectos de la deuda: escudo fiscal y liquidación de la firma" (2014) propone un modelo para valorar empresas teniendo como base la teoria de opciones reales.
...
option_single_period_binomial_tree <- function(S, K, rf, u, d){
  ...
  p <- (1+rf - d) / (u - d)
  # Precio del activo subyacente en el estado de subida y bajada
  Su <- S * u
  Sd <- S * d
  # Valor de la opción en el estado de subida y bajada
  Cu <- max(Su - K, 0)
  Cd <- max(Sd - K, 0)
...
Comparando los resultados, en Black and Scholes el precio de la opción es 60 pesos y con el modelo binomial 137 pesos. Sin embargo, cuando en black and scholes se considera tiempo como 1 año, y en el modelo binomial Su como 1.1 y Sd como 0.9 los resultados son parecidos, dando como resultado 124 pesos en black and sholes y 137 en el arbol binomial.

La principal diferencia es que el modelo binomial incluye la probabilidad de que el activo se encuentre en el estado up, y cuanto seria el retorno en ese caso. Por ejemplo, puede existir un 70% que la acción suba de precio, y su retorno va a ser 1.2 veces el precio original.

Modelo Black & Scholes considera tiempo lo cual permite analizar

Modelo binomial considera la probabilidad y rentabilidad en los distintos escenarios, lo cual es util ya que al saber de antam

Otra diferencias entre Black and Scholes y el modelo binomial es que el primero requiere una solución de ecuación diferencial estocastica mientras que el modelo binomial es un metodo estadistico (Ahmad, 2018)

black shcoles solo europeo

binomial deja americana

### Reporte Grupal: Caso Administración

De los tres casos presentados se decidio replicar "Política de crédito de Braam Industries" para el activo Banco Santander
...
Los bancos conmunmente no tienen una tasa de incumplimiento, por ende, se considero la tasa de morosidad. Los bancos hacen una diferenciación dependiendo del tipo de cliente.
...
Sin embargo, la CMF publicó la morosidad a 90 dias donde no separa por los distintos tipos de deudores. Para efectos de este trabajo se consideró el valor de 2,17% que pertence al total. El periodo de cuentas por cobrar se considero 90 dias. Para los costos administrativos se consideraro el total de los gastos operacionales y este se dividio por la cantidad de activos y se multiplico por 100 para tener el porcentaje.

La tasa de interes anual actualmente es 9,5%, lo que equivale a 0,024867% interes diario.
